...
object Day14:
  ...
  case class Memory(address: Int, value: BigInt)
    
  object Memory:
    def of(command: String): Memory =
  ...
  def part1(data: InputType): Long =
    val map = collection.mutable.Map[Int, BigInt]()
    ...
    println(map)
    map.map(_._2.toLong).reduce(_ + _)
  ...
  def maskEach(mask: String, command: String): Memory =
    val org = Memory.of(command)
    val binStr = org.value.toString(2)
    val result = 
    ...
    println(result)
    Memory(org.address, BigInt.apply(result, 2))
