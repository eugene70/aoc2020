...
object Day07:
   ...
   * @return
   */
  private def part1(implicit rules: InputType): Long =
    rules.count(m =>
      m._2.contains("shiny gold") || m._2.keySet.exists(k => travers(k)))

  private def travers(bag: String)(implicit rules: InputType): Boolean =
    rules.get(bag).exists(m =>
      m.contains("shiny gold") || m.keySet.exists(k => travers(k)))
   ...
   * @return
   */
  private def part2(implicit rules: InputType): Long =
    travers2("shiny gold")

  private def travers2(bag: String)(implicit rules: InputType): Int =
    rules.get(bag)
