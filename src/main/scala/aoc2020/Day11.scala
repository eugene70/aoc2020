...
object Day11:
  ...
  object SeatStatus:
    def of(ch: Char): SeatStatus =
      ch match
        case 'L' => Empty
        case '.' => Floor
        case '#' => Occupied
  
  type SeatTable = IndexedSeq[IndexedSeq[SeatStatus]]

  def equals(a: SeatTable, b: SeatTable): Boolean =
    a.flatten.zip(b.flatten).forall(t => t._1 == t._2)
  ...
  private def ready(input: String): SeatTable =
    input
      .linesIterator
      .map(_.toList.map(SeatStatus.of(_)).toIndexedSeq)
      .toIndexedSeq

  private def part1(data: SeatTable): Long =
    var tgtData = data
    while
      val orgData = tgtData
      tgtData = trans(orgData, neighbors1, transOne1)
      !equals(orgData, tgtData)
    do()
    tgtData.flatten.count(_ == Occupied)

  private def part2(data: SeatTable): Long =
    var orgData = data
    var tgtData = data
    while
      orgData = tgtData
      tgtData = trans(orgData, neighbors2, transOne2)
      !equals(orgData, tgtData)
    do()
    tgtData.flatten.count(_ == Occupied)

  private def trans(implicit data: SeatTable,
                    neighbors: (Int, Int, SeatTable) => Neighbors,
                    transOne: (SeatStatus, Neighbors) => SeatStatus): SeatTable =
    var copy: SeatTable = IndexedSeq.empty
    for (i <- data.indices)
      var line: IndexedSeq[SeatStatus] = IndexedSeq.empty
      for (j <- data(i).indices)
        line = line.appended(
          data(i)(j) match
            case Floor => Floor
            case _ => transOne(data(i)(j), neighbors(i, j, data))
        )
      copy = copy.appended(line)
    copy

  private def transOne1(seat: SeatStatus, neighbors: Neighbors): SeatStatus =
    seat match
      case Empty => if (!neighbors.contains(Occupied)) Occupied else Empty
      case Occupied => if (neighbors.count(_ == Occupied) >= 4) Empty else Occupied
      case Floor => Floor

  private def transOne2(seat: SeatStatus, neighbors: Neighbors): SeatStatus =
    seat match
      case Empty => if (!neighbors.contains(Occupied)) Occupied else Empty
      case Occupied => if (neighbors.count(_ == Occupied) >= 5) Empty else Occupied
      case Floor => Floor

  private def neighbors1(i: Int, j: Int, data: SeatTable): Neighbors =
      (IndexedSeq.empty :+
        at(i - 1, j - 1)(data) :+
        at(i - 1, j)(data)  :+
        at(i - 1, j + 1)(data) :+
        ...
        at(i + 1, j - 1)(data) :+
        at(i + 1, j)(data) :+
        at(i + 1, j + 1)(data)).flatten
  
  private def neighbors2(i: Int, j: Int, data: SeatTable): Neighbors =
    (IndexedSeq.empty :+
      fromDirection(i, j)(-1, -1)(data) :+
      fromDirection(i, j)(-1, 0)(data) :+
      fromDirection(i, j)(-1, 1)(data) :+
      fromDirection(i, j)(0, -1)(data) :+
      fromDirection(i, j)(0, 1)(data) :+
      fromDirection(i, j)(1, -1)(data) :+
      fromDirection(i, j)(1, 0)(data) :+
      fromDirection(i, j)(1, 1)(data)).flatten

  private def at(i: Int, j:Int)(implicit data: SeatTable): Option[SeatStatus] =
    if (data.indices.contains(i) && data(i).indices.contains(j))
      return Some(data(i)(j))
    None

  private def fromDirection(i: Int, j: Int)(di: Int, dj: Int)
                           (implicit data: SeatTable): Option[SeatStatus] =
    at(i + di, j + dj) match
      case Some(Floor) =>
        val nextDi = if (di == 0) 0 else di + di / Math.abs(di)
        val nextDj = if (dj == 0) 0 else dj + dj / Math.abs(dj)
        fromDirection(i, j)(nextDi, nextDj)
      case s => s
