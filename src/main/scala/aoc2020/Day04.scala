...
object Day04:
  ...
  private def part1(data: InputType): Long =
    data.count(passport => 
      mandatoryFields.forall(field => passport.map(_._1).contains(field)
    ))
  
  private def part2(data: InputType): Long =
    val default: String => Boolean = _ => false
    data
      .filter(passport => mandatoryFields.forall(field => passport.map(_._1).contains(field)))
      .count(passport => passport.forall(field => validators.getOrElse(field._1, default)(field._2)))
